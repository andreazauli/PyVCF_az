...
class _vcf_metadata_parser(object):
    ...
    def __init__(self):
        ...
        self.info_pattern = re.compile(r'''\#\#INFO=<
            ...
            (?:,\s*Version="?(?P<version>[^"]*)"?)?
            >''', re.VERBOSE)
        # Patched (VAPOR, 2026-07-14): fallback, order-independent parser for
        # ##INFO lines whose ID/Number/Type/Description attributes appear in
        # a valid-but-nonstandard order (e.g. Type before Number, as seen in
        # some SV-aware VCFs). Used only when info_pattern above fails to
        # match, so behavior for standard-order lines is unchanged. Original
        # file backed up alongside this one as parser.py.bak-vapor-20260714.
        self.info_attr_pattern = re.compile(
            r'(\w+)=("(?:[^"\\]|\\.)*"|[^,>]*)')
    ...
    def read_info(self, info_string):
        ...
        # Fallback: order-independent parse (see info_attr_pattern above).
        body_match = re.match(r'^\#\#INFO=<(.*)>\s*$', info_string)
        if body_match:
            attrs = dict(self.info_attr_pattern.findall(body_match.group(1)))
            if 'ID' in attrs and 'Number' in attrs and 'Type' in attrs:
                id_val = attrs['ID']
                type_val = attrs['Type']
                desc_val = attrs.get('Description', '').strip('"')
                source_val = attrs.get('Source')
                if source_val is not None:
                    source_val = source_val.strip('"')
                version_val = attrs.get('Version')
                if version_val is not None:
                    version_val = version_val.strip('"')
                num = self.vcf_field_count(attrs['Number'])
                info = _Info(id_val, num, type_val, desc_val,
                             source_val, version_val)
                return (id_val, info)
